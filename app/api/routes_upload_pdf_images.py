...
def add_manifest_item(
    ...
    manifest["items"].append({
        ...
        "web_path": f"{base_url.rstrip('/')}/uploads/{user_id}/{pdf_stem}/images/{img_path.name}",
        "is_visible": True
    })
        ...
        for xref, *_ in images:
            ...
            for r in image_rects:
                try:
                    ...
                    if rect_area(r2) > page_area * 0.90:
                        continue
...
def extract_raw_images_to_manifest(pdf_path: FilePath, out_dir: FilePath, json_path: FilePath, user_id: str, base_url: str) -> Dict[str, Any]:
    ensure_dir(out_dir)
    images_dir = out_dir / "images"
    ensure_dir(images_dir)
    ...
    for pi, page in enumerate(doc, 1):
        ...
        for image_info in images:
            ...
            try:
                ...
                img_path = images_dir / f"p{pi}_img{idx}.{ext}"
...
async def handle_pdf_upload_raw(
    id: str,
    file: UploadFile,
    background_tasks: Optional[BackgroundTasks],
    authorization: Optional[str],
    ...
    with dest_pdf.open("wb") as f:
        shutil.copyfileobj(file.file, f)
    result = extract_raw_images_to_manifest(dest_pdf, pdf_dir, dest_json, user_id=id, base_url=base_url)
    rel_json = result["json_path"].relative_to(BASE_DIR).as_posix()
    result_url = f"{base_url.rstrip('/')}/{rel_json}"
    return {
        "id": id,
...
@router.post("/{user_id}/raw")
async def upload_pdf_images_raw(
    user_id: str = Path(..., description="User ID of the uploader"),
    file: UploadFile = File(...),
    background_tasks: BackgroundTasks = BackgroundTasks(),
    authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ...
    try:
        base_url = os.getenv("BASE_URL", "http://localhost:8000")
        result = await handle_pdf_upload_raw(user_id, file, background_tasks, authorization, base_url)
        if "error" in result:
            raise HTTPException(status_code=422, detail=result["error"])
        return JSONResponse(result)
    except HTTPException:
