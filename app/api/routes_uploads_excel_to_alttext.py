...
def _force_visual_type_prefix(text: str, vtype: str, short: bool) -> str:
    ...
    t = re.sub(
        r"^(A|An|The)\s+(" + "|".join(re.escape(v) for v in VT_WORDS) + r")\s+",
        "",
        t,
        flags=re.I,
    )
...
def choose_visual_type(item: Dict[str, Any]) -> str:
    ...
    path = (
        (item.get("image_path") or "") + " " + " ".join(item.get("image_paths", []))
    ).lower()
    hay = f"{name} {path}"
...
def openai_alt_text(
    ...
    try:
        logger.info(f"Starting OpenAI alt text generation for image: {img_path}")
        logger.info(f"OPENAI_MODEL: {OPENAI_MODEL}")

        if not OPENAI_API_KEY:
            logger.error("OPENAI_API_KEY is missing")
            return clean_and_validate_alt(
                default_s, default_l, vtype, short_limit, long_limit
            )
        ...
        logger.info(f"OpenAI parsed response for {img_path}: {txt}")
        ...
        if data is None and "{" in txt and "}" in txt:
            ...
            try:
                ...
                logger.error(
                    f"JSON parse failed from OpenAI response: {str(parse_error)}"
                )
                data = None
        ...
        if isinstance(data, dict):
            ...
            logger.error(
                f"No valid JSON object found in OpenAI response for image: {img_path}"
            )
        ...
        logger.error(
            f"OpenAI alt text generation failed for {img_path}: {str(e)}", exc_info=True
        )
        return clean_and_validate_alt(
            default_s, default_l, vtype, short_limit, long_limit
        )
    ...
    try:
        ...
        if not excel_stem:
            ...
            if not src_url:
                raise HTTPException(
                    status_code=400,
                    detail="Provide excel_stem or source_excel_url to locate the manifest",
                )
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Unable to infer excel_stem from source_excel_url",
                )
        ...
        by_row = {
            it.get("row"): it for it in existing_items if it.get("row") is not None
        }
        by_index = {
            it.get("index"): it for it in existing_items if it.get("index") is not None
        }
        by_name = {
            it.get("file_name"): it for it in existing_items if it.get("file_name")
        }
        ...
        for it in to_generate:
            ...
            s, l = openai_alt_text(img_path, vtype, vis_text, short_limit, long_limit)
            it["short_alt_text"] = s
            it["long_alt_text"] = l
            it["short_alt"] = s
            it["long_alt"] = l
...
async def regenerate_alt_text(payload: dict = Body(...)):
    ...
    try:
        ...
        if page is not None:
            for it in items:
                if int(it.get("index", -9999)) == int(idx) and int(
                    it.get("page", -9999)
                ) == int(page):
                    target = it
                    break
        ...
        if target is None:
            raise HTTPException(
                status_code=404, detail=f"Item not found for index={idx} (page={page})"
            )
        ...
        if not img_path:
            raise HTTPException(
                status_code=404, detail="No valid image path for this item"
            )

        vtype = choose_visual_type(target)
        vis_text = target.get("visible_text", "")
        short_alt, long_alt = openai_alt_text(
            img_path, vtype, vis_text, short_limit, long_limit
        )
        ...
        return JSONResponse(
            {
                ...
                or (target.get("image_urls", [None]) or [None])[0],
                "short_alt": short_alt,
                "long_alt": long_alt,
                "visual_type": vtype,
            }
...
async def upload_excel_to_alttext(
    ...
    try:
        ...
        if ext not in ["xlsx", "xls", "csv"]:
            raise HTTPException(
                status_code=400, detail="Only Excel (.xlsx/.xls) or CSV files allowed"
            )
        ...
        if ext in ["xlsx", "xls"]:
            ...
            for i, row in enumerate(
                ...
                for j, b in enumerate(blobs, start=1):
                    try:
                        ...
                        ext2 = (
                            ".png"
                            if fmt not in ["JPEG", "JPG", "PNG", "GIF", "BMP", "WEBP"]
                            else f".{fmt.lower() if fmt!='JPG' else 'jpg'}"
                        )
            ...
            for i, r in df.iterrows():
                row_map = {
                    str(c): ("" if pd.isna(r[c]) else str(r[c])) for c in df.columns
                }
                row_map["image_url"] = ""
                row_map["image_urls"] = []
                row_map["image_paths"] = []
...
async def download_current_excel_alt_text_file(
    user_id: int = Path(..., description="User ID of the uploader"),
    body: Dict[str, Any] = Body(...),
    db: Session = Depends(get_db),
):
    try:
        if not body or "excel_stem" not in body or "format" not in body:
            raise HTTPException(status_code=400, detail="Missing excel_stem or format")

        new_body = dict(body)
        new_body["user_id"] = user_id

        new_body.pop("project_id", None)
        new_body.pop("project_name", None)
        new_body.pop("project_description", None)

        return await export_excel_alt_text(new_body, db)
...
async def export_excel_alt_text(
    ...
    try:
        ...
        if "user_id" not in body or "excel_stem" not in body or "format" not in body:
            raise HTTPException(
                status_code=400, detail="Missing user_id, excel_stem or format"
            )
        ...
        if not items:
            raise HTTPException(
                status_code=400, detail="No eligible items found (visible & index > 0)"
            )

        project_id = body.get("project_id")
        if (
            not project_id
            and body.get("project_name")
            and body.get("project_description")
        ):
        ...
        def _fname(it):
            return (it.get("file_name") or "").strip()

        def _imgurl(it):
            return (it.get("image_url") or it.get("url") or "").strip()

        def _short(it):
            return it.get("short_alt") or it.get("short_alt_text") or ""

        def _long(it):
            return it.get("long_alt") or it.get("long_alt_text") or ""
        ...
        if fmt == "json":
            ...
            with zipfile.ZipFile(tmpf.name, "w") as zf:
                ...
                for i, it in enumerate(items, 1):
                    ...
                    if img_loc and img_loc.exists():
                        ...
                        manifest_entries.append(
                            f'<item id="img{i}" href="{embedded_name}" media-type="{media}"/>'
                        )
                    ...
                    zf.writestr(
                        xhtml_path, page_html, compress_type=zipfile.ZIP_DEFLATED
                    )
                    manifest_entries.append(
                        f'<item id="{xhtml_id}" href="Text/page_{i}.xhtml" media-type="application/xhtml+xml"/>'
                    )
                    spine_entries.append(f'<itemref idref="{xhtml_id}"/>')
                    nav_list_items.append(
                        f'<li><a href="Text/page_{i}.xhtml">{title_txt}</a></li>'
                    )
                ...
                zf.writestr(
                    "OEBPS/nav.xhtml", nav_html, compress_type=zipfile.ZIP_DEFLATED
                )
                manifest_entries.append(
                ...
                zf.writestr(
                    "OEBPS/content.opf", content_opf, compress_type=zipfile.ZIP_DEFLATED
                )
            ...
            with zipfile.ZipFile(tmpf.name, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                for i, it in enumerate(items, 1):
                    ...
                    if img_loc and img_loc.exists():
                        ...
                        mime = (
                            "image/jpeg"
                            if img_loc.suffix.lower() in (".jpg", ".jpeg")
                            else "image/png"
                        )
        ...
        public_url = (
            f"{PUBLIC_BASE_URL}/uploads/{user_id}/{excel_stem}/{download_filename}"
        )
        ...
        if project_id and ProjectFile is not None:
            try:
                ...
                pf = ProjectFile(
                    ...
                    ext=ext,
                    project_type="Excel to Alt Text",
                    size_bytes=size,
                    checksum=None,
                    is_deleted=False,
                    created_at=now,
...
"""),
                    {
                        "image_used": (user_plan.image_used or 0)
...
"""),
        ...
        return FileResponse(
            path=str(final_path), media_type=media_type, filename=download_filename
        )
